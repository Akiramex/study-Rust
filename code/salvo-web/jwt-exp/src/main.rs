...
async fn main() {
    ...
    Server::new(acceptor)
        .serve(Router::with_hoop(auth_handler).goal(index).push(Router::new().hoop(is_login).push(Router::with_path("test").goal(test))))
        .await;
}

#[handler]
async fn is_login(res: &mut Response, depot: &mut Depot, ctrl: &mut FlowCtrl){
